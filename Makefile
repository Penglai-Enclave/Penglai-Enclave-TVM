...
$(opensbi): $(linux_image) $(sdk)
	...
	make clean && \
	make PLATFORM=generic FW_PAYLOAD_PATH=${linux_image} FW_FDT_PATH=${linux_dtb2}
...
.PHONY: qemu
qemu: $(qemu)
	$(qemu) -nographic -M virt -m 512M -smp 4  -kernel $(opensbi)/build/platform/generic/firmware/fw_payload.elf \
